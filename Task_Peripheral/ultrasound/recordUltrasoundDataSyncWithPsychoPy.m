...
% ==== INIT CAMERAS ====
uscam = webcam(1);       % Ultrasound cam 1 (new/affected)
rwcam = webcam(2);       % Real-world cam
uscam2 = webcam(3);  % Ultrasound cam 2
...
info_out = lsl_streaminfo(lib, 'us_frame_outlet', 'Markers', 1, fps, 'cf_string', 'us_frame_id');
...
parfor i=1:numWorkers
    ...
    saveRepeatFiles('./scratch', testfilename, dummyData, dummyData, dummyData);
end
...
% make global and ref
trialTime = 3.6;
numFrames = ceil(trialTime * fps); % a bit loarger than needed
...
allTrialData = struct('us', usFrames, 'us2', us2Frames, 'rw', rwFrames); % set to x(:, :) = 0 every time func call ends
        ...
        if ~isempty(marker)
            ...
            switch m % case switch
                ...
                case 'GO_ONSET'
                    ...
                    allTrialData = recordRepeatFrames(outlet, uscam, rwcam, uscam2, trialTime, fps, finalRes, fullResRW, cropBounds, allTrialData);
                    repeatCount = repeatCount + 1;
                    tstamp = datestr(now,'yyyymmdd_HHMMSS_FFF');
                    outlet.push_sample({tstamp});
                    baseName = sprintf('grasp-%s_rep-%d_%s', currentGrasp, repeatCount, tstamp);
                    f = parfeval(@saveRepeatFiles, 0, outputDir, baseName, allTrialData.us, allTrialData.rw, allTrialData.us2);
